...
class VerifyEmailController {
  ...
  async handle(req, res) {
    ...
    // Feature Flag: Se verificação de email está desabilitada
    if (!isEmailVerificationEnabled()) {
      return res.status(200).send(
        verificationResultTemplate({
          success: true,
          message:
            "A verificação de email está desabilitada neste ambiente. Sua conta foi criada como verificada e você já pode acessar o sistema.",
        }),
      );
    }
    ...
    try {
      ...
      return res.status(200).send(
        verificationResultTemplate({
          success: true,
          message:
            "Seu endereço de e-mail foi verificado com sucesso. Você já pode acessar sua conta e aproveitar a Digital Store!",
        }),
      );
    } catch (error) {
      const statusCode = error.statusCode || 400;
      const message =
        error.message || "Não foi possível validar seu e-mail. O link pode estar expirado ou ser inválido.";
